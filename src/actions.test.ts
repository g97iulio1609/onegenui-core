...
describe("action helper", () => {
  ...
  describe("throttled", () => {
    it("creates action with throttle", () => {
      const a = action.throttled("scroll", 100);

      expect(a.name).toBe("scroll");
      expect(a.throttleMs).toBe(100);
    });

    it("creates action with throttle and params", () => {
      const a = action.throttled("scroll", 250, { direction: "up" });

      expect(a.throttleMs).toBe(250);
      expect(a.params).toEqual({ direction: "up" });
    });
  });

  describe("debounced", () => {
    it("creates action with debounce", () => {
      const a = action.debounced("search", 300);

      expect(a.name).toBe("search");
      expect(a.debounceMs).toBe(300);
    });

    it("creates action with debounce and params", () => {
      const a = action.debounced("search", 500, { query: "test" });

      expect(a.debounceMs).toBe(500);
      expect(a.params).toEqual({ query: "test" });
    });
  });

  describe("withRetry", () => {
    it("creates action with retry config", () => {
      const a = action.withRetry("upload", {
        maxAttempts: 3,
        delayMs: 1000,
        backoffMultiplier: 2,
      });

      expect(a.name).toBe("upload");
      expect(a.retry).toEqual({
        maxAttempts: 3,
        delayMs: 1000,
        backoffMultiplier: 2,
      });
    ...
    it("creates action with retry and params", () => {
      const a = action.withRetry(
        "upload",
        { maxAttempts: 5 },
        { file: "test.txt" },
      );

      expect(a.retry?.maxAttempts).toBe(5);
      expect(a.params).toEqual({ file: "test.txt" });
    });
  });

  describe("optimistic", () => {
    it("creates action with optimistic update", () => {
      const a = action.optimistic("like", {
        path: "/posts/1/liked",
        value: true,
      });

      expect(a.name).toBe("like");
      expect(a.optimistic).toEqual({
        path: "/posts/1/liked",
        value: true,
      });
    });

    it("creates action with optimistic and revertOnError", () => {
      const a = action.optimistic("like", {
        path: "/likes",
        value: 100,
        revertOnError: true,
      ...
      expect(a.optimistic?.revertOnError).toBe(true);
    });

    it("creates action with optimistic and params", () => {
      const a = action.optimistic(
        "like",
        { path: "/liked", value: true },
        { postId: "123" },
      );

      expect(a.params).toEqual({ postId: "123" });
      expect(a.optimistic?.path).toBe("/liked");
    });
  });
});
