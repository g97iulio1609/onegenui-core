...
describe("setByPath", () => {
  ...
  it("handles array-like paths", () => {
    ...
    expect((data.items as unknown[])[0]).toBe("first");
  });

  it("pushes to arrays with '-'", () => {
    const data: Record<string, unknown> = { items: [] };

    setByPath(data, "/items/-", "first");
    setByPath(data, "/items/-", "second");

    expect(data.items).toEqual(["first", "second"]);
  });

  it("creates arrays for indexed paths", () => {
    const data: Record<string, unknown> = {};

    setByPath(data, "/items/0/title", "Task A");

    expect(data.items).toEqual([{ title: "Task A" }]);
  });
});
