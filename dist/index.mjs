...
import { getByPath as getByPath2 } from "@onegenui/utils";
...
var deviceTypeSchema = z2.enum(["mobile", "tablet", "desktop"]);
var VisibilityConditionSchema = z2.union([
  z2.boolean(),
  z2.object({ path: z2.string() }),
  z2.object({ auth: z2.enum(["signedIn", "signedOut"]) }),
  z2.object({ role: z2.union([z2.string(), z2.array(z2.string())]) }),
  z2.object({ feature: z2.union([z2.string(), z2.array(z2.string())]) }),
  z2.object({
    device: z2.union([deviceTypeSchema, z2.array(deviceTypeSchema)])
  }),
...
var ActionRetryConfigSchema = z3.object({
  maxAttempts: z3.number().int().positive(),
  delayMs: z3.number().int().positive().optional(),
  backoffMultiplier: z3.number().positive().optional(),
  maxDelayMs: z3.number().int().positive().optional()
});
var ActionOptimisticConfigSchema = z3.object({
  path: z3.string(),
  value: z3.unknown(),
  revertOnError: z3.boolean().optional()
});
var ActionSchema = z3.object({
  ...
  throttleMs: z3.number().int().positive().optional(),
  debounceMs: z3.number().int().positive().optional(),
...
var UIElementMetaSchema = z6.object({
  turnId: z6.string().describe("Unique ID for the AI turn that created this"),
  sequence: z6.number().int().nonnegative().describe("Sequence number within turn"),
  createdAt: z6.number().describe("Unix timestamp of creation"),
  updatedAt: z6.number().optional().describe("Unix timestamp of last update")
});
var StreamPatchSchema = z6.object({
  op: z6.enum(["add", "replace", "remove", "move", "copy", "test"]),
  path: z6.string().regex(/^\//, "Path must start with /"),
  value: z6.unknown().optional(),
  from: z6.string().optional()
});
var PatchMessageSchema = z6.object({
  type: z6.literal("patch"),
  patches: z6.array(StreamPatchSchema),
  targetPath: z6.string().optional()
});
var ChatMessageSchema = z6.object({
  type: z6.literal("message"),
  role: z6.enum(["user", "assistant", "system"]),
  content: z6.string(),
  id: z6.string().optional()
});
var QuestionMessageSchema = z6.object({
  type: z6.literal("question"),
  ...
  inputType: z6.enum(["text", "select", "multiselect", "number", "date"]).optional(),
  required: z6.boolean().optional()
});
var SuggestionMessageSchema = z6.object({
  type: z6.literal("suggestion"),
  suggestions: z6.array(
    z6.object({
      id: z6.string(),
      label: z6.string(),
...
var ToolProgressMessageSchema = z6.object({
  type: z6.literal("tool-progress"),
  toolId: z6.string(),
  toolName: z6.string(),
  status: z6.enum(["pending", "running", "complete", "error"]),
  progress: z6.number().min(0).max(100).optional(),
  message: z6.string().optional(),
  result: z6.unknown().optional()
});
var StreamControlSchema = z6.object({
  type: z6.literal("control"),
  action: z6.enum(["start", "end", "error", "abort"]),
  error: z6.object({
    code: z6.string(),
    message: z6.string(),
    recoverable: z6.boolean()
  }).optional()
});
var StreamMessageSchema = z6.discriminatedUnion("type", [
  PatchMessageSchema,
  ChatMessageSchema,
...
var StreamFrameSchema = z6.object({
  version: z6.literal("2.0").describe("Protocol version"),
  timestamp: z6.number().describe("Unix timestamp"),
  correlationId: z6.string().uuid().describe("Request correlation ID"),
  sequence: z6.number().int().nonnegative().describe("Frame sequence number"),
  message: StreamMessageSchema
});
var UIElementBaseSchema = z6.object({
  key: z6.string().describe("Unique element key (REQUIRED)"),
  type: z6.string().describe("Component type from catalog (REQUIRED)"),
  props: z6.record(z6.string(), z6.unknown()).describe("Component props (REQUIRED)"),
  parentKey: z6.string().optional(),
  layout: z6.object({
    grid: z6.object({
      column: z6.number().optional(),
      row: z6.number().optional(),
      columnSpan: z6.number().optional(),
      rowSpan: z6.number().optional()
    }).optional(),
    size: z6.object({
      width: z6.union([z6.number(), z6.string()]).optional(),
      height: z6.union([z6.number(), z6.string()]).optional(),
      minWidth: z6.union([z6.number(), z6.string()]).optional(),
      maxWidth: z6.union([z6.number(), z6.string()]).optional(),
      minHeight: z6.union([z6.number(), z6.string()]).optional(),
      maxHeight: z6.union([z6.number(), z6.string()]).optional()
    }).optional(),
    resizable: z6.boolean().optional()
  }).optional(),
  visible: z6.boolean().optional(),
  locked: z6.boolean().optional(),
  _meta: UIElementMetaSchema.optional()
});
var UIElementSchema = UIElementBaseSchema.extend({
  children: z6.lazy(() => z6.array(UIElementSchema)).optional()
});
